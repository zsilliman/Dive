...
shared_ptr<Goal> Goal::allocWithTexture(shared_ptr<Texture> texture) {
    shared_ptr<Goal> goal = make_shared<Goal>();
    goal->_node = PolygonNode::allocWithTexture(texture);
    return goal;
}

void Goal::setScale(float x, float y) {
    _node->setScale(x, y);
}

void Goal::setInitialPosition(float x, float y) {
    _node->setPositionX(x);
    _node->setPositionY(y);
    _initial_pos = Vec2(x, y);
}

void Goal::initPhysics(shared_ptr<ObstacleWorld> world) {
    _body = BoxObstacle::alloc(Vec2(_node->getPositionX(), _node->getPositionY()), Size(_node->getWidth(), _node->getHeight()));
    _body->setBodyType(b2BodyType::b2_staticBody);
    _body->setDensity(0.0f);
...
void Goal::parallaxTranslate(float reference_x, float reference_y, float reference_dx) {
...
void Goal::setMapSize(int x_start, int x_end) {
